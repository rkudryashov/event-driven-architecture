...
import com.romankudryashov.eventdrivenarchitecture.bookservice.persistence.InboxMessageRepository
import com.romankudryashov.eventdrivenarchitecture.bookservice.persistence.entity.InboxMessageEntity
import com.romankudryashov.eventdrivenarchitecture.bookservice.service.InboxMessageService
...
class InboxMessageServiceImpl(
    ...
    override fun markInboxMessagesAsReadyForProcessingByInstance(batchSize: Int): Int {
        fun saveReadyForProcessing(inboxMessage: InboxMessageEntity) {
            ...
            if (inboxMessage.status != InboxMessageEntity.Status.New) throw BookServiceException("Inbox message with id=${inboxMessage.id} is not in 'New' status")
    ...
    override fun process(inboxMessage: InboxMessageEntity) {
        ...
        if (inboxMessage.status != InboxMessageEntity.Status.ReadyForProcessing)
            throw BookServiceException("Inbox message with id=${inboxMessage.id} is not in 'ReadyForProcessing' status")
        if (inboxMessage.processedBy == null)
            throw BookServiceException("'processedBy' field should be set for an inbox message with id=${inboxMessage.id}")
